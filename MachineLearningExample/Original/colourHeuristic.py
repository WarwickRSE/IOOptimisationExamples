...
def categorise_and_file(filename, prefix, outdir, im, quiet=False):
    """Accepts a PIL image, identifies the dominant colour name and copies prefix+filename to outdir_`colourname`/filename"""
...
def sort_n_ims(prefix, outdir, count, quiet=False):
    ...
    for i in range(count):
        ...
        categorise_and_file(filename, prefix, outdir, im, quiet)

def categorise_single(full_filename, plot=True):
...
if __name__ == "__main__":
    ...
    if mode in ["show", "identify"]:
        ...
        filename = argv[2]
        if mode == "show":
            categorise_single(filename, plot=True)
        else:
            categorise_single(filename, plot=False)
    elif mode in ["sort"]:
        ...
        try:
            ...
            exit()
        sort_n_ims(in_dir, out_dir, count, quiet=False)       
